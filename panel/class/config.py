  ...
  `addtime` INTEGER
    ...
    def GetPanelSSL(self, get):
        ...
        if os.path.exists(ssl_path + 'baota_root.pfx') and not os.path.exists(ssl_path + 'baota_root.crt'):
            ...
            if not os.path.exists("{}/baota_root.crt".format(ssl_path)):
                shell = 'openssl pkcs12 -in {ssl_path}/baota_root.pfx -clcerts -nokeys -out {ssl_path}/baota_root.crt {passwd}'.format(
                    ssl_path=ssl_path, passwd=passwd)
                public.print_log(public.ExecShell(shell))
    ...
    def read_dedicated_servicer(self):
        cache_file = "{}/data/dedicated_servicer.json".format(self._setup_path)
        user_info_str = public.readFile("{}/data/userInfo.json".format(self._setup_path))
        default_data = {
            "url":"https://work.weixin.qq.com/kfid/kfc2736fb7043e75033?enc_scene=ENCEjXjodtXFKqcXx9fEmWnwYdLZnx29uinWVD9NuMrgxC9",
            "type":"link",
            "servicerName":""
        }
        try:
            if os.path.isfile(cache_file) and time.time() - os.path.getmtime(cache_file) < 84600:
                data = json.loads(public.readFile(cache_file))
                if "url" in data:
                    return data
        except:
            pass
        ...
        try:
            ...
            _api = 'https://www.bt.cn/api/v2/weixin_worker/dedicated_servicer'
            resp = public.HttpPost(_api, data=pdata)
            public.print_log(resp)
            resp_data = json.loads(resp)
            if "url" in resp_data["res"]:
                public.writeFile(cache_file, json.dumps(resp_data["res"]))
