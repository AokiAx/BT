...
class main(panelBase):
    ...
    def get_public_config(self, args):
        ...
        data['dedicated_servicer'] = _config_obj.read_dedicated_servicer()
        return data
    ...
    # 获取公告
    def get_announcement_list(self, get):
        announcement_data = {"list": [], "count": 0}
        try:
            if "p" in get:
                p = int(get.p)
            else:
            ...
            data = certModel.main().new_ssl_proxy_request({"url": "/api/v1/ssl/announcement/get_list", "p": p, "limit": rows})
            if data['status']:
                announcement_data = data['msg']
                announcement_list = announcement_data['list']
                # 获取已读列表
                try:
                    read_list = json.loads(public.readFile("/www/server/panel/data/announcement_read.json"))
                except:
                    read_list = []
                public.print_log(read_list)
                public.print_log(announcement_list)
                # 设置已读状态
                for announcement in announcement_list:
                    if announcement['id'] in read_list:
    ...
    # 获取公告详情
    def get_announcement_detail(self, get):
        try:
            announcement_id = int(get.announcement_id)
        except:
            return public.returnMsg(False, "参数错误")
        try:
            from sslModel import certModel
            data = certModel.main().new_ssl_proxy_request({"url": "/api/v1/ssl/announcement/get_detail", "id": announcement_id})
            if not data['status']:
                return public.returnMsg(False, data['msg'])
            # 设置已读状态
            set_read = self.mark_announcement_as_read(get)
            if not set_read['status']:
                public.print_log("标记公告为已读失败: {}".format(set_read['msg']))
            return data
        except:
            public.print_log(public.get_error_info())
            return public.returnMsg(False, "获取公告详情失败")

    # 标记为已读
    def mark_announcement_as_read(self, get):
        try:
            announcement_id = int(get.announcement_id)
        except:
            return public.returnMsg(False, "参数错误")
        try:
            ...
            read_list = []
        if announcement_id not in read_list:
            read_list.append(announcement_id)
            public.writeFile("/www/server/panel/data/announcement_read.json", json.dumps(read_list))
        return public.returnMsg(True, "已标记公告为已读")

    # 全部标记为已读
    def mark_all_announcements_as_read(self, get):
        try:
            get.rows = 9999
            announcement_list = self.get_announcement_list(get)['list']
            read_list = [announcement['id'] for announcement in announcement_list]
            public.writeFile("/www/server/panel/data/announcement_read.json", json.dumps(read_list))
            return public.returnMsg(True, "已标记所有公告为已读")
        except:
            public.print_log(public.get_error_info())
            return public.returnMsg(False, "标记所有公告为已读失败")
