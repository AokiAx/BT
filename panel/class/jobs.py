...
def aliyun_bailian_trace():
    '''
        @name 百炼平台埋点
        @return void
    '''
    bt_agent_plugin_dir = '/www/server/panel/mod/project/agent'
    product_id_file = '{}/aliyun_product_id.pl'.format(bt_agent_plugin_dir)
    account_id_file = '{}/aliyun_account_id.pl'.format(bt_agent_plugin_dir)
    if os.path.exists(product_id_file) and os.path.exists(account_id_file): return

    o_file = '/www/server/panel/data/o.pl'
    if not os.path.exists(o_file): return

    o_content = public.readFile(o_file)
    aliyun_tags = ('aliyun_lts', 'aliyun_latest', 'aliyun_ecswp', 'aliyun', 'aliyunwp')
    if not isinstance(o_content, str) or o_content.strip() not in aliyun_tags: return

    if not os.path.exists(bt_agent_plugin_dir):
        os.makedirs(bt_agent_plugin_dir)

    try:
        product_id, _ = public.ExecShell('curl -s http://100.100.100.200/latest/meta-data/image/market-place/product-code')
        product_id = product_id.strip() if product_id else ''
        if not product_id or '404 - Not Found' in product_id: return

        account_id, _ = public.ExecShell('curl -s http://100.100.100.200/latest/meta-data/owner-account-id')
        account_id = account_id.strip() if account_id else ''

        public.writeFile(product_id_file, product_id)
        public.writeFile(account_id_file, account_id)
    except:
        pass
