...
class database(datatool.datatools):
    ...
    def AddDatabase(self, get):
        ...
        re_list = re.findall(r"[，。？！；：“”‘’（）【】《》￥&\u4e00-\u9fa5]+", data_pwd)
...
'''
    ...
    def SetupPassword(self, get):
        ...
        re_list = re.findall(r"[，。？！；：“”‘’（）【】《》￥&\u4e00-\u9fa5]+", password)
        if re_list: return public.returnMsg(False, f'数据库密码不能包含中文和特殊字符 {" ".join(re_list)}')
    ...
    def ResDatabasePassword(self, get):
        ...
        re_list = re.findall(r"[，。？！；：“”‘’（）【】《》￥&\u4e00-\u9fa5]+", newpassword)
    ...
    def ToBackup(self, get):
        ...
        try:
            ...
            if os.path.isfile(export_dir):
                zip_cmd = ["zip", "-m", "-j"]
                if zip_password:
                    zip_cmd.extend(["-P", zip_password])
                zip_cmd.extend([backup_path, export_dir])
                with open("/tmp/backup_sql_{}.log".format(db_id), "ab") as log_file:
                    subprocess.run(zip_cmd, stdout=log_file, stderr=log_file)
            elif os.path.isdir(export_dir):
                zip_cmd = ["zip", "-m", "-r"]
                if zip_password:
                    zip_cmd.extend(["-P", zip_password])
                zip_cmd.extend([backup_path, file_name])
                with open("/tmp/backup_sql_{}.log".format(db_id), "ab") as log_file:
                    subprocess.run(zip_cmd, cwd=db_backup_dir, stdout=log_file, stderr=log_file)
            else:
    ...
    def InputSql(self, get):
        ...
        if file_ext == ".zip":
            ...
            # result = subprocsess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            unzip_cmd = ["unzip"]
            if zip_password:
                unzip_cmd.extend(["-P", zip_password])
            unzip_cmd.extend(["-o", file, "-d", input_dir])
            result = subprocess.run(unzip_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            # print(command)
    ...
    def BinLog(self, get):
        ...
        log_bin_status = re.search(r"\nlog-bin", mysql_cnf)
        is_off_bin_log = re.search(r"\nskip-log-bin", mysql_cnf)
...
USE `{db_name}`;
        ...
        for user_item in user_data:
            ...
            for host_item in host_data:
                ...
                for access_item in access_data:
                    re_obj = re.search(r"GRANT\s+([^.]+)\s+ON\s+([^.]+)\.([^.]+)\s+TO", access_item[0], flags=re.IGNORECASE)
                    if re_obj:
    ...
    def export_table_structure(self,get):
        import subprocess
        import os
        public.exists_args("db_name,table_name,filename", get)

        db_name = get.db_name.strip()
        table_name = get.table_name.strip()
        filename = get.filename.strip()
        if not db_name or not table_name or not filename:
            return public.returnMsg(False, "参数不能为空")
        if any(ord(i) < 32 for i in filename):
            return public.returnMsg(False, "导出路径不合法")
        if not public.path_safe_check(filename):
            return public.returnMsg(False, "导出路径不合法")

        default_export_dir = os.path.join(public.get_backup_path(), "database_structure")
        if os.path.isabs(filename):
            export_path = os.path.realpath(filename)
        else:
            export_path = os.path.realpath(os.path.join(default_export_dir, filename))
        if not export_path.endswith(".sql"):
            export_path += ".sql"

        export_dir = os.path.dirname(export_path)
        if not os.path.basename(export_path):
            return public.returnMsg(False, "导出路径不合法")
        deny_roots = [
            "/etc", "/root", "/boot", "/bin", "/sbin", "/usr", "/proc", "/sys",
            "/dev", "/var", "/lib", "/lib64", public.GetConfigValue("setup_path"),
            public.get_panel_path()
        ]
        for deny_root in deny_roots:
            deny_root = os.path.realpath(deny_root)
            if export_path == deny_root or export_path.startswith(deny_root + os.sep):
                return public.returnMsg(False, "不能导出到系统关键目录")
        if os.path.islink(export_path):
            return public.returnMsg(False, "导出路径不能是软链接")
        if not os.path.exists(export_dir):
            os.makedirs(export_dir, 0o700)
        if os.path.islink(export_dir):
            return public.returnMsg(False, "导出目录不能是软链接")
        if os.path.exists(export_path):
            export_base, export_ext = os.path.splitext(export_path)
            if not export_ext:
                export_ext = ".sql"
            timestamp = time.strftime("%Y%m%d%H%M%S", time.localtime())
            export_path = "{}_{}{}".format(export_base, timestamp, export_ext)
            if os.path.exists(export_path):
                export_path = "{}_{}_{}{}".format(export_base, timestamp, public.GetRandomString(5), export_ext)
        ...
        try:
            ...
            db_name_for_sql = db_name.replace("`", "``")
            tables = self.map_to_list(self.DB_MySQL.query("show tables from `{}`".format(db_name_for_sql)))
        ...
        db_password = public.M("config").where("id=?", (1,)).getField("mysql_root")
        env = os.environ.copy()
        env["MYSQL_PWD"] = str(db_password or "")
        cmd = [
            self._MYSQLDUMP_BIN,
            "-u", "root",
            "-d",
        ...
        try:
            ...
            fd = os.open(export_path, open_flags, 0o644)
            created_file = True
            with os.fdopen(fd, "wb") as fp:
                process = subprocess.Popen(cmd, stdout=fp, stderr=subprocess.PIPE, env=env)
                _, stderr = process.communicate()
            if process.returncode == 0:
                return public.returnMsg(True, "表结构已成功导出到文件: {}".format(export_path))
            if created_file and os.path.exists(export_path):
                os.remove(export_path)
            return public.returnMsg(False, "导出表结构失败: {}".format(stderr.decode(errors="ignore")))
        except Exception as ex:
            if created_file and os.path.exists(export_path):
                try:
                    os.remove(export_path)
                    ...
                    pass
            return public.returnMsg(False, "导出表结构失败: {}".format(str(ex)))
