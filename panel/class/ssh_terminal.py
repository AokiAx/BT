...
class local_ssh_terminal(ssh_terminal):
    ...
    class _LocalShell:
        ...
        @staticmethod
        def _preexec_fn():
            import fcntl, termios, signal

            # 1. 创建新会话，脱离父进程的进程组（等同于 start_new_session=True 的效果）
            os.setsid()

            # 2. 显式将当前的 stdin (已经被 Popen 绑定为 PTY slave) 设为控制终端
            #    这是让 Ctrl+C、Ctrl+Z 信号生效的关键
            try:
            ...
            # 【可选 but 推荐】忽略 SIGHUP 信号
            # 这样即使面板重启导致 PTY 主端关闭，bash 也不会因为收到 SIGHUP 而意外退出
            # 能最大程度保证“重启面板自身时命令仍能继续执行”
...
''' % (not_debug_str)
        ...
        def __init__(self):
            ...
            self.proc = subprocess.Popen(
                ...
                preexec_fn=self._preexec_fn,  # ← 改为使用 preexec_fn
            )
