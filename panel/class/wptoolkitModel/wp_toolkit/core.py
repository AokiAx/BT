...
class wpmgr:
    ...
    def run_wp_with_cli(self, php_code: str, extra_data: typing.Union[typing.Dict, typing.List] = (),
            ...
            # 结果接收文件
            tmp_out_file = '{}.out'.format(tmp_file)

            # PHP代码
            s = r'''<?php
    define('AAP_CLI', true);
    define('AAP_ABSPATH', '{wp_root_path}/');
    define('AAP_WPINC', '{wpinc}/');
    define('AAP_OUT_FILE', '{out_file}');
    ...
    function _aap_exit() {{
    	// 正常响应
    	$buffered_resp = ob_get_clean();

    	// 将结果写入文件
    	if (!empty(AAP_OUT_FILE)) {{
    		file_put_contents(AAP_OUT_FILE, $buffered_resp);
    		exit();
    	}}
    ...
    add_filter('wp_die_ajax_handler', function($handler) {{
        return function($message, $title, $args) {{
            ...
            // 响应错误
            if ((200 < $args['response'] || $args['response'] > 399) && !empty($message)) {{
                _aap_fail($message);
            }}
    ...
    '''.format(admin_id=self.retrieve_administrator_id()) if not without_auth else '')
            ...
            try:
                # 没有安装sudo时，尝试安装
                if not os.path.exists('/usr/bin/sudo') and not os.path.exists('/usr/sbin/sudo'):
                    if os.path.exists('/usr/bin/apt') or os.path.exists('/usr/sbin/apt'):
                        public.ExecShell("apt install sudo -y >> /tmp/schedule.log")
                    else:
                        public.ExecShell("yum install sudo -y >> /tmp/schedule.log")

                # 优先使用sudo
                if os.path.exists('/usr/bin/sudo') or os.path.exists('/user/sbin/sudo'):
                    # 使用www用户执行
                ...
                if err:
                    ...
                    if m:
                        ...
                        raise public.HintException(
                            public.get_msg_gettext('PHP运行出错: {}. 请修改PHP版本后重试', (m.group(1),)))

                # 从输出文件中读取返回值
                if os.path.exists(tmp_out_file):
                    with open(tmp_out_file, 'r', encoding='utf8') as fp:
                        return fp.read()
