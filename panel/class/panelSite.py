...
'''
        ...
        for data in websites_info:
            ...
            try:
                ...
                if get.sql == 'true':
                    ...
                    create_other['db_user'] = get.datauser = public.gen_password(6)
                    create_other['db_status'] = True
                if get.ftp == 'true':
                    create_other['ftp_pass'] = get.ftp_password = public.gen_password(16)
                    create_other['ftp_user'] = get.ftp_username = public.gen_password(6)
    ...
    def AddSite(self, get, multiple=None):
        ...
        ps_domain = siteMenu['domain'].strip().split(':')[0]
        # 判断是否为ipv6
...
#HTTP_TO_HTTPS_END'''
        ...
        try:
            if not os.path.exists("{}/data/ignore_upgrade_cert.pl".format(public.get_panel_path())) and cert_data:
                if type == 3:
                    ...
                    details = []
                    for detail in act_data["data"][0]["detail"]:
                    ...
                    if details:
                        res["upgrade_cert"] = "免费SSL证书即将到期，29.9元起续费商用SSL证书"
                        res["upgrade_cert_details"] = details
                    else:
                        res["upgrade_cert"] = ""
            else:
                res["upgrade_cert"] = ""
    ...
    def get_ssl_push_status(self, get, siteName=None, stype=None,ssl_status=None, repair=False):
        if get:
            siteName = get.siteName
        ...
        if not ssl_task or not site_id:
            ...
            if isinstance(task_data_project, str) and repair is False:
                from mod.base.push_mod import update_ssl_task_for_11_7
                try:
                    update_ssl_task_for_11_7()
                except:
                    pass
                return self.get_ssl_push_status(get, siteName=siteName, stype=stype,ssl_status=ssl_status, repair=True)
            elif isinstance(task_data_project, str) and repair is True:
                try:
                    os.remove("{}/data/mod_push_data/update_panel.pl".format(public.get_panel_path()))
                except:
                    pass
                return {
                    'task_data': {
...
#PROXY-END%s'''
        ...
        if not os.path.exists(ng_proxydir):
            public.Mkdir(ng_proxydir, 0o755, "P")
