...
class bt_task:
  ...
  `addtime`			INTEGER
    ...
    # 执行 unzip 解压，并把 unzip 的输出写到原来的日志文件里
    def _run_unzip_to_log(self, sfile, dfile, password, log_file):
        import subprocess
        if password is None:
            password = ''
        ...
        try:
            with open(log_file, 'wb') as log_fp:
                return subprocess.call(
                    ['unzip', '-X', '-P', password, '-o', sfile, '-d', dfile],
                    stdout=log_fp,
    ...
    def _unzip(self, sfile, dfile, password, log_file, power=None, user=None):
        ...
        if sfile[-4:] == '.zip':
            ...
            self._run_unzip_to_log(sfile, dfile, password, log_file)
