...
class LocationTools(_IndexBlockTools):
    ...
    def set_security_referer(
        ...
        if not status:
            if loc:
                loc.block.directives = [
                    ...
                    if not sub_d.get_name() in ("expires", "valid_referers", "if", "access_log", "proxy_pass")
                ]
        ...
        if not loc:
            ...
            if proxy_pass_copy:
                block.directives.insert(0, proxy_pass_copy)
            ...
            if not loc.top_find_directives("proxy_pass") and proxy_pass_copy:
                loc_block = loc.get_block()
                if loc_block:
                    loc_block.directives.insert(0, proxy_pass_copy)
