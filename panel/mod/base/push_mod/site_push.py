...
class ProjectStatusTask(BaseTask):
    ...
    def get_push_data(self, task_id: str, task_data: dict) -> Optional[dict]:
        ...
        if int(task_data["count"]) == 1:
            ...
            if self._to_project_model(task_data["cycle"]) == "nodejsModel":
                sql = DB("sites")
                web_info = sql.where(
                    "project_type = ? and id = ?",
                    (self._to_project_type(task_data["cycle"]), task_data["project"])
                ).field("project_config").find()
                try:
                    project_config = json.loads(web_info["project_config"])
                    ...
                    if project_type == "pm2":
                        ...
                        from mod.project.nodejs.nodeMod import main
                        model_main_obj = main()
                except:
                    pass
