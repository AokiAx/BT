...
class main(JournalctlManage, SecureManage):
    ...
    def index_ssh_info(self, get):
        ...
        try:
            if get.get("log_type", "") == "Failed":
                flag = self.login_failed_flag
            elif get.get("log_type", "") == "Accepted":
                flag = self.login_access_flag
            else:
                flag = self.login_all_flag
            ...
            today_count = self.get_secure_log_count(flag, today_str)
            yesterday_count = self.get_secure_log_count(flag, yesterday_str)
        except Exception as e:
            import traceback
