...
class RestoreManager(SiteModule,DatabaseModule,FtpModule,DataManager,BaseUtil,ConfigManager):
    ...
    def restore_data(self,timestamp, force_restore: str):
        ...
        if "site" in backup_data_list:
            try:
                self.restore_site_data(timestamp, force_restore)
            except:
                pass
...
if __name__ == '__main__':
    ...
    if hasattr(restore_manager, method_name):  # 检查方法是否存在
        ...
        if method_name == "restore_data":
            # 还原方法必须传第二个参数
            if len(sys.argv) < 4:
                print("restore_data 需要参数：timestamp force_restore")
                sys.exit(1)
            force_restore = sys.argv[3]
            method(timestamp, force_restore)
        else:
            method(timestamp)
    else:
