...
class SiteModule(DataManager,BaseUtil,ConfigManager):
    ...
    def restore_site_data(self, timestamp: str, force_restore: str) -> None:
        """还原站点数据
        ...
        for site in restore_data['data_list']['site']:
            try:   
                ...
                if 'site_db_record' in site:
                    new_id = self.restore_site_db_data(site['site_db_record'], force_restore)
                    if new_id is None:
                        continue
                # 还原域名记录
                if 'domains' in site:
                    ...
                    domain_sql.table('domain')
                    pid = new_id
                    if 'id' in site['domains']:
                        del site['domains']['id']
                    try:
                        # 1. 查询当前站点(pid)下 已存在的所有域名 + 对应主键id
                        exist_list = domain_sql.field("id,name").where("pid=?", pid).select()
                        self.print_log(f"已有域名数据: {exist_list}", 'restore')
                        # 2. 构建 域名->域名ID 映射表
                        exist_map = {}
                        for idx, row in enumerate(exist_list):
                            # 只处理合法的 [id, name] 子列表
                            if isinstance(row, dict):
                                d_name = str(row.get("name", "")).strip()
                                d_id = row.get("id")
                                if d_name and d_id is not None:
                                    exist_map[d_name] = d_id
                        # 3. 统一转为列表遍历（兼容 site['domains'] 是字典/列表两种格式）
                        domain_items = site['domains']
                        item_list = []
                        if isinstance(domain_items, dict):
                            item_list = list(domain_items.values())
                        else:
                            item_list = domain_items
                        # 4. 循环处理每一条域名
                        for idx, item in enumerate(item_list):
                            if not isinstance(item, dict):
                                continue
                            item["pid"] = pid  # 绑定所属站点ID
                            domain_name = item.get("name", "").strip()

                            # 5. 判断：有重复就更新，没有就插入
                            if domain_name in exist_map:
                                # 重复：根据域名id 更新覆盖数据
                                d_id = exist_map[domain_name]
                                update_data = {
                                    "name": domain_name,
                                    "pid": pid,
                                    "port": item.get("port", 0)
                                }
                                domain_sql.where("id=?", d_id).update(update_data)
                            else:
                                insert_data = {
                                    "name": item["name"],
                                    "pid": item["pid"],
                ...
                #还原网站文件
                site_zip = site_backup_path + site_name + ".zip"
                if os.path.exists(site_zip):
                    public.ExecShell("cd {site_backup_path} && unzip -o {site_name}.zip".format(site_backup_path=site_backup_path,site_name=site_name))
                    backup_path = "{site_backup_path}/{site_name}/".format(site_backup_path=site_backup_path,site_name=site_name)
                    public.ExecShell("cd {site_backup_path} && \cp -rpa {site_name}/*  /www/wwwroot/{site_name}".format(site_backup_path=site_backup_path,site_name=site_name))
    ...
    def restore_site_db_data(self,site_db_record:dict, force_restore:int):
        new_id = None
        import db
        sql = db.Sql()
        sql.table('sites')
        ...
        target_name = site_db_record["name"]
        #检查存在就不插入，返回之前id
        try:
            data = sql.field("id").where("name=?", target_name).select()
            if data and len(data) > 0:
                first_item = data[0]
                # 兼容字典 / 列表 两种格式
                if isinstance(first_item, dict):
                    new_id = first_item.get("id")
            if new_id is not None and force_restore == '1':
                return new_id
            if new_id is not None and force_restore == '0':
                return None
        except Exception as e:
            print(f"插入前查询失败: {str(e)}")

        try:
            # 插入新记录
            insert_row_id = sql.insert(site_db_record)
            sql.commit()
            ...
            try:
                ...
                # 取出名称
                unique_val = site_db_record["name"]
                result = sql.field("id").where("name=?", unique_val).select()
