...
class main(DataManager,BaseUtil,ConfigManager):
    ...
    def add_restore(self,get=None):
        if not hasattr(get, "timestamp"):
            return self.return_data(False,"参数错误","参数错误")
        timestamp=get.timestamp
        if not hasattr(get, "force_restore"):
            return self.return_data(False,"参数错误","参数错误")
        force_restore=get.force_restore
        public.ExecShell("nohup btpython /www/server/panel/mod/project/backup_restore/restore_manager.py restore_data {} {} > /dev/null 2>&1 &".format(int(timestamp), int(force_restore)))
